...
function SearchInput({ fetchData, userLocation }) {
  const [location, setLocation] = useState("");
  //   console.log(userLocation);

  const handleSearch = async (loc) => {
    Promise.all([getCurrentWeatherData(loc), getNewsArticles(loc)]).then(
      ([weather, news]) => {
    ...
    <>
      <div className="flex mx-auto gap-16">
        ...
        <div className="flex flex-1 ">
          ...
          />
          <button
            className="flex items-center p-3 border rounded-r-lg hover:bg-gray-400 bg-gray-50"
        ...
        <button
          ...
          className="flex items-center p-3 border rounded-lg bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-40  border-gray-100"
        >
          <svg
