          ...
          <div className="px-6 py-6 relative rounded-lg bg-white bg-opacity-50">
            <div className="flex mb-4 justify-between items-center">
              <div>
                ...
                <h6 className="mb-0">{getDate(weather.dt)}</h6><small>{weather.weather.description}</small>
              </div>
              <div>
                <img src={`https://openweathermap.org/img/wn/${weather.weather[0].icon}@2x.png`} className="block w-24 h-24" alt='' />
                <p className="text-center text-gray-500 text-sm">Cloudy</p>
              </div>
              <div className="text-right">
                <h3 className="font-bold text-4xl mb-0"><span>{weather.main.temp}&deg;C</span></h3>
              </div>
            </div>
            <div className="block sm:flex justify-between items-center flex-wrap">
              <div className="w-full sm:w-1/2">
                <div className="flex mb-2 gap-8 items-center"><span>Temp</span><small className="px-2 inline-block">{weather.main.temp}&nbsp;&deg;C</small></div>
              </div>
              <div className="w-full sm:w-1/2">
                <div className="flex mb-2 gap-8 items-center"><span>Feels like</span><small className="px-2 inline-block">{weather.main.feels_like}&nbsp;&deg;C</small></div>
              </div>
              <div className="w-full sm:w-1/2">
                <div className="flex mb-2 gap-8 items-center"><span>Temp min</span><small className="px-2 inline-block">{weather.main.temp_min}&nbsp;&deg;C</small></div>
              </div>
              <div className="w-full sm:w-1/2">
                <div className="flex mb-2 gap-8 items-center"><span>Temp max</span><small className="px-2 inline-block">{weather.main.temp_max}&nbsp;&deg;C</small></div>
              </div>
            </div>
          </div>
